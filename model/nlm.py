    ...
    def calWeights(self, img, kernel, y, x):
        ...
        for j in range(2 * self.Ds + 1 - 2 * self.ds - 1):
            for i in range(2 * self.Ds + 1 - 2 * self.ds - 1):
                ...
                neighbour_w = img[start_y - self.ds:start_y +
                                  self.ds + 1, start_x - self.ds:start_x + self.ds + 1]
    ...
    def execute(self):
        ...
        for y in range(img_pad.shape[0] - 2 * self.Ds):
            for x in range(img_pad.shape[1] - 2 * self.Ds):
                center_y = y + self.Ds
                center_x = x + self.Ds
                sweight, average, wmax = self.calWeights(
                    img_pad, kernel, center_y, center_x)
